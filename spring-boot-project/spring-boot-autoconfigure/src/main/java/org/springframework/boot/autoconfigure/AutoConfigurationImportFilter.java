...
public interface AutoConfigurationImportFilter {
	 ...
	 * values in this array. —— 正在考虑的自动配置类。此数组可能包含 {@code null} 元素。实现不应更改此数组中的值。
	 *
	 *                                 从spring.factories中获取到的配置类
	 *
	 * @param autoConfigurationMetadata access to the meta-data generated by the
	 * auto-configure annotation processor —— 访问自动配置注释处理器生成的元数据
	 *
	 *                                  从spring-autoconfigure-metadata.properties中获取到的"配置类被加载的条件"
	 *
